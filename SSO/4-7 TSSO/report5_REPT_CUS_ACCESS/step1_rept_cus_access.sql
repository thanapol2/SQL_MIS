SELECT ''                                        AS id,
  ...
  to_char(add_months(tran_date, 6516), 'dd-mon-yyyy') as ACCESS_DATE_ID,
  userid_cus_cnt,
  access_cus_cnt,
...
FROM
  ...
  LEFT JOIN
    ...
    FROM
      ( SELECT DISTINCT TRUNC(to_date(ACCESS_DTM,'mm/dd/yyyy')) tran_date,
        login_id as userid
      FROM tsso_access_log
      WHERE TRUNC(to_date(ACCESS_DTM,'mm/dd/yyyy'), 'dd') BETWEEN TO_DATE('22/02/2015', 'dd/mm/yyyy') AND NEXT_DAY(TO_DATE('22/02/2015', 'dd/mm/yyyy'),'SATURDAY' )
      ) user_a
    GROUP BY user_a.tran_date
    ) userid
  ON weekly.tran_date = userid.tran_date
  LEFT JOIN
    (SELECT TRUNC(to_date(ACCESS_DTM,'mm/dd/yyyy')) tran_date,
