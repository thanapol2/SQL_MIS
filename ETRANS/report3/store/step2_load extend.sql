                                 ...
                                 FROM
                                     (
                                         ...
                                         WHERE
                                             status = 'APPROVE'
-- check expire date
                                             AND months_between(trunc(TO_DATE('01/02/2019', 'dd/mm/yyyy')), trunc(updated_date)) BETWEEN
                                             10 AND 12  
--and MONTHS_BETWEEN(trunc(sysdate),updated_Date) between 10 and 12
                                     ) old
                                     LEFT JOIN token_request new ON old.username = new.username
-- check rew new date
                                                                    AND new.created_date BETWEEN add_months(trunc(old.updated_date
                                                                    ), 10) AND add_months(trunc(old.updated_date), 12)
